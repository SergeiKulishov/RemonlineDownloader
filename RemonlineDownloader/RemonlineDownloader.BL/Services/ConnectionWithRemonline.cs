...
namespace RemskladDesktop
{
    public class ConnectionWithRemonline
    {
        ...
        public static async Task<string> GetOrders(int pages = 1)
        {
            string token = await GetToken();
            string url = $"https://api.remonline.ru/order/" +
                         $"?token={token}" + $"&branches[]=22732&page={pages}";
        ...
        public static async Task<Root> GetListOfOrders(int page = 1)
        {
            var OrdersJson = await GetOrders(page);
        ...
        public static async Task<string> GetRawJsonOrders(int page)
        {
